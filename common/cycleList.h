...
public:
	...
	/// <summary>
	/// Вставляет элемент по указанному номеру.
	/// </summary>
	/// <param name="num">номер, по которому нужно вставить элемент.</param>
	/// <param name="data">Данные, которые нужно вставить в список.</param>
	/// <exception cref="out_of_range">Выбрасывается, если номер выходит за пределы списка.</exception>
	
	void insert(int num, T data)
	{
		if (num < 0 || num > counter)
		{
			throw out_of_range("номер вне диапазона");
		}
		if (num == counter)
		{
			add(data);
	...
	/// <summary>
	/// Удаляет элемент по указанному номер.
	/// </summary>
	/// <param name="num">номер элемента, который нужно удалить.</param>
	/// <exception cref="out_of_range">Выбрасывается, если номер выходит за пределы списка.</exception>
	...
	/// <summary>
	/// Получает элемент по указанному номеру.
	/// </summary>
	/// <param name="num">номер элемента, который нужно получить.</param>
	...
	T& operator[](const int num)
	{
		if (num < 0 || num >= counter)
		{
			throw out_of_range("номер вышел за пределы");
		}
		Node* numNode = head;
		for (int i = 0; i < num; i++)
		{
